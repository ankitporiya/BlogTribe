  ...
  const fetchMyBlogs = async (page = 1) => {
    try {
      ...
      let response = await fetch(
        `${API_BASE_URL}/api/blogs/user/me/blogs?page=${page}`,
        {
          headers: {
      ...
      if (!response.ok && response.status === 404) {
        response = await fetch(
          `${API_BASE_URL}/api/blogs/user/blogs?page=${page}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
  ...
  const handleUpdateBlog = async () => {
    try {
      ...
      if (data.success) {
        ...
        setBlogs(
          blogs.map((blog) => (blog._id === editingBlog._id ? data.blog : blog))
        );
    ...
    <div className="min-h-screen py-8 px-4 animate-fade-in">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 animate-slide-in-up">
          <h1
            className="text-3xl font-bold mb-4 animate-slide-in-left"
            style={{ color: "#A64D79" }}
          >
            My Blogs
          </h1>
          <p className="text-gray-300 animate-slide-in-left animate-delay-100">
        ...
        {successMessage && (
          <div
            className="mb-6 border rounded-lg p-4 flex items-center animate-slide-in-up"
        ...
        {/* Search and Filter */}
        <div
          className="rounded-lg shadow-sm p-6 mb-6 animate-slide-in-up animate-delay-200"
        ...
        {loading ? (
          ...
          <div
            className="text-center py-12 rounded-lg shadow-sm animate-scale-in"
            style={{ backgroundColor: "#3B1C32" }}
          >
            <div
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredBlogs.map((blog, index) => (
              <div
                key={blog._id}
                className={`blog-card rounded-lg shadow-sm overflow-hidden animate-slide-in-up ${
                  index % 3 === 0
                    ? "animate-delay-100"
                    : index % 3 === 1
                    ? "animate-delay-200"
        ...
        {pagination.totalPages > 1 && (
          <div className="mt-8 flex justify-center animate-slide-in-up animate-delay-400">
            <div className="flex space-x-2">
              ...
              <span
                className="px-4 py-2 text-white rounded-md animate-scale-in"
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 modal-backdrop">
          <div
            className="rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto modal-content"
            style={{ backgroundColor: "#3B1C32" }}
          >
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold" style={{ color: "#A64D79" }}>
                  Edit Blog
                </h2>
                <button
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 modal-backdrop">
          <div
            className="rounded-lg max-w-md w-full p-6 modal-content"
            style={{ backgroundColor: "#3B1C32" }}
          >
            <div className="flex items-center mb-4">
              <AlertCircle className="h-6 w-6 text-red-500 mr-2" />
              <h3 className="text-lg font-medium" style={{ color: "#A64D79" }}>
                Delete Blog
              </h3>
            </div>
            <p className="text-gray-300 mb-6">
