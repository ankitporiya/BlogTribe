...
const getAllBlogs = async (req, res) => {
  try {
    ...
    console.log('📋 Get All Blogs - Found blogs:', blogs.length, 'Total:', total);
...
const updateBlog = async (req, res) => {
  try {
    ...
    if (blog.author.toString() !== userId && req.user.role !== 'admin') {
      console.log('❌ Update Blog - Not authorized. Blog author:', blog.author.toString(), 'User ID:', userId);
      return res.status(403).json({
        success: false,
        message: 'Not authorized to update this blog'
      });
...
const deleteBlog = async (req, res) => {
  try {
    ...
    const userId = getUserId(req);
    if (blog.author.toString() !== userId && req.user.role !== 'admin') {
      console.log('❌ Delete Blog - Not authorized. Blog author:', blog.author.toString(), 'User ID:', userId);
      return res.status(403).json({
        success: false,
        message: 'Not authorized to delete this blog'
      });
...
const getUserBlogs = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && requestingUserId !== userId) {
      query.published = true;
      console.log('👤 Get User Blogs - Not admin/owner, filtering to published only');
    }
    ...
    console.log('👤 Get User Blogs - Found blogs:', blogs.length, 'Total:', total);
